...
if __name__ == "__main__":
    ...
    for kfold, (train_idx, valid_idx) in enumerate(kf.split(dataset)):
        ...
        for epoch in tqdm(range(epochs)):
            ...
            writer.add_scalars(main_tag=f'LOSS_{kfold+1}', 
                               tag_scalar_dict={'train_loss':loss_per_epoch_train,
                                                'valid_loss':loss_per_epoch_valid},
                               global_step=epoch)
            
            writer.add_scalars(main_tag=f'RP_{kfold+1}', 
                               tag_scalar_dict={'train_rp':train_rp,
                                                'valid_rp':valid_rp},
                               global_step=epoch)
            
            writer.add_scalars(main_tag=f'RS_{kfold+1}', 
                               tag_scalar_dict={'train_rs':train_rs,
                                                'valid_rs':valid_rs},
                               global_step=epoch)
            
            writer.add_scalars(main_tag=f'RMSE_{kfold+1}', 
                               tag_scalar_dict={'train_rmse':train_rmse,
